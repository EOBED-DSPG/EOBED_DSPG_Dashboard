...
# # Add counties to map
# add25Counties <- function(map, df, group = NULL, fc){
#    # Set up highlight label text (HTML formatting)
#    miLab <- paste("Median Income: $",floor(df$X25_to_44),sep = "")
#    moeLab <- paste("Margin of Error: $",floor(as.numeric(df$MOE_25_to_45)),sep = "")
#    labels <- sprintf(
...
# Add tracts to map
add25Tracts <- function(map, df, group = NULL, fc){
   # Set up highlight label text (HTML formatting)
   miLab <- paste("Median Income: $",floor(df$X25_to_44),sep = "")
   moeLab <- paste("Margin of Error: $",floor(as.numeric(df$MOE_25_to_45)),sep = "")
   labels <- sprintf(
...
plot25Inc <- function( tract_merge, groups = NULL, pal, myDomain, cityLng,
   ...
   miMap<-leaflet() %>%
      ...
      # add25Counties(df = county_merge[[1]], group = groups[1], fc = ~pal(X25_to_44)) %>%   #2012 County Data
      add25Tracts(df = tract_merge[[1]], group  = groups[1], fc = ~pal(X25_to_44)) %>%     #2012 Tract Data
      
      # add25Counties(df = county_merge[[2]], group = groups[3], fc = ~pal(X25_to_44)) %>%   #2013 County Data
      add25Tracts(df = tract_merge[[2]], group  = groups[2], fc = ~pal(X25_to_44)) %>%     #2013 Tract Data
      
      # add25Counties(df = county_merge[[3]], group = groups[5], fc = ~pal(X25_to_44)) %>%   #2014 County Data
      add25Tracts(df = tract_merge[[3]], group  = groups[3], fc = ~pal(X25_to_44)) %>%     #2014 Tract Data
      
      # add25Counties(df = county_merge[[4]], group = groups[7], fc = ~pal(X25_to_44)) %>%   #2015 County Data
      add25Tracts(df = tract_merge[[4]], group  = groups[4], fc = ~pal(X25_to_44)) %>%     #2015 Tract Data
      
      # add25Counties(df = county_merge[[5]], group = groups[9], fc = ~pal(X25_to_44)) %>%   #2016 County Data
      add25Tracts(df = tract_merge[[5]], group  = groups[5], fc = ~pal(X25_to_44)) %>%    #2016 Tract Data
      
      # add25Counties(df = county_merge[[6]], group = groups[11], fc = ~pal(X25_to_44)) %>%  #2017 County Data
      add25Tracts(df = tract_merge[[6]], group  = groups[6], fc = ~pal(X25_to_44)) %>%    #2017 Tract Data
      
      # add25Counties(df = county_merge[[7]], group = groups[13], fc = ~pal(X25_to_44)) %>%  #2018 County Data
      add25Tracts(df = tract_merge[[7]], group  = groups[7], fc = ~pal(X25_to_44)) %>%    #2018 Tract Data
      ...
      addLegend(pal = pal,                                          # Use given color palette
                ...
                title = "Individual Median Income Age 25-44",         # Legend Title
...
get25Tracts <- function(ald, myTracts){
   ...
   for(i in 1:length(ald)){
      ...
      tract_merge[[i]]$X25_to_44 <- as.numeric(tract_merge[[i]]$X25_to_44)
   }
   
   return(tract_merge)
}

# get25Counts <- function(ald, myCounts){
...
#       
#       # Make usable data points numeric rather than chars
#       county_merge[[i]]$X25_to_44 <- as.numeric(county_merge[[i]]$X25_to_44)
#    }
#    return(county_merge)
# }
...
create25 <- function(){
   ...
   myDomain <- minRelevantIncome:max(as.numeric(ald[[7]]$X25_to_44),na.rm = T)
   ...
   # county_merge <- get25Counts(ald, myCounts)
   tract_merge <- get25Tracts(ald, myTracts)
   
   
   plot25Inc( tract_merge = tract_merge, groups = groups, pal = pal, myDomain = myDomain, cityLng = cityLng,
