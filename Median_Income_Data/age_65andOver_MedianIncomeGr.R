...
# Add counties to map
# add65Counties <- function(map, df, group = NULL, fc){
#    # Set up highlight label text (HTML formatting)
#    miLab <- paste("Median Income: $",floor(df$X65_and_over),sep = "")
#    moeLab <- paste("Margin of Error: $",floor(as.numeric(df$MOE_65_and_over)),sep = "")
#    labels <- sprintf(
...
# Add tracts to map
add65Tracts <- function(map, df, group = NULL, fc){
   # Set up highlight label text (HTML formatting)
   miLab <- paste("Median Income: $",floor(df$X65_and_over),sep = "")
   moeLab <- paste("Margin of Error: $",floor(as.numeric(df$MOE_65_and_over)),sep = "")
   labels <- sprintf(
...
plot65Inc <- function(county_merge, tract_merge, groups = NULL, pal, myDomain, cityLng,
   ...
   miMap<-leaflet() %>%
      ...
      # add65Counties(df = county_merge[[1]], group = groups[1], fc = ~pal(X65_and_over)) %>%   #2012 County Data
      add65Tracts(df = tract_merge[[1]], group  = groups[1], fc = ~pal(X65_and_over)) %>%     #2012 Tract Data
      
      # add65Counties(df = county_merge[[2]], group = groups[3], fc = ~pal(X65_and_over)) %>%   #2013 County Data
      add65Tracts(df = tract_merge[[2]], group  = groups[2], fc = ~pal(X65_and_over)) %>%     #2013 Tract Data
      
      # add65Counties(df = county_merge[[3]], group = groups[5], fc = ~pal(X65_and_over)) %>%   #2014 County Data
      add65Tracts(df = tract_merge[[3]], group  = groups[3], fc = ~pal(X65_and_over)) %>%     #2014 Tract Data
      
      # add65Counties(df = county_merge[[4]], group = groups[7], fc = ~pal(X65_and_over)) %>%   #2015 County Data
      add65Tracts(df = tract_merge[[4]], group  = groups[4], fc = ~pal(X65_and_over)) %>%     #2015 Tract Data
      
      # add65Counties(df = county_merge[[5]], group = groups[9], fc = ~pal(X65_and_over)) %>%   #2016 County Data
      add65Tracts(df = tract_merge[[5]], group  = groups[5], fc = ~pal(X65_and_over)) %>%    #2016 Tract Data
      
      # add65Counties(df = county_merge[[6]], group = groups[11], fc = ~pal(X65_and_over)) %>%  #2017 County Data
      add65Tracts(df = tract_merge[[6]], group  = groups[6], fc = ~pal(X65_and_over)) %>%    #2017 Tract Data
      
      # add65Counties(df = county_merge[[7]], group = groups[13], fc = ~pal(X65_and_over)) %>%  #2018 County Data
      add65Tracts(df = tract_merge[[7]], group  = groups[7], fc = ~pal(X65_and_over)) %>%    #2018 Tract Data
      ...
      addLegend(pal = pal,                                          # Use given color palette
                ...
                title = "Individual Median Income Age 65 and Over",         # Legend Title
...
get65Tracts <- function(ald, myTracts){
   ...
   for(i in 1:length(ald)){
      ...
      tract_merge[[i]]$X65_and_over <- as.numeric(tract_merge[[i]]$X65_and_over)
   }
   
   return(tract_merge)
}

# get65Counts <- function(ald, myCounts){
...
#       
#       # Make usable data points numeric rather than chars
#       county_merge[[i]]$X65_and_over <- as.numeric(county_merge[[i]]$X65_and_over)
#    }
#    return(county_merge)
# }
...
create65 <- function(){
   ...
   myDomain <- minRelevantIncome:max(as.numeric(ald[[7]]$X65_and_over),na.rm = T)
   ...
   # county_merge <- get65Counts(ald, myCounts)
   tract_merge <- get65Tracts(ald, myTracts)
   
   
   plot65Inc( tract_merge = tract_merge, groups = groups, pal = pal, myDomain = myDomain, cityLng = cityLng,
