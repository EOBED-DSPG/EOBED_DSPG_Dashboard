...
# Add counties to map
# addCounties <- function(map, df, group = NULL, fc){
#    # Set up highlight label text (HTML formatting)
#    miLab <- paste("Median Income: $",floor(df$average_Income),sep = "")
#    moeLab <- paste("Margin of Error: $",floor(as.numeric(df$average_MOE)),sep = "")
#    labels <- sprintf(
...
# Add tracts to map
addTracts <- function(map, df, group = NULL, fc){
   # Set up highlight label text (HTML formatting)
   miLab <- paste("Median Income: $",floor(df$average_Income),sep = "")
   moeLab <- paste("Margin of Error: $",floor(as.numeric(df$average_MOE)),sep = "")
   labels <- sprintf(
...
plotAvInc <- function( tract_merge, groups = NULL, pal, myDomain, cityLng,
   ...
   miMap<-leaflet() %>%
      ...
      # addCounties(df = county_merge[[1]], group = groups[1], fc = ~pal(average_Income)) %>%   #2012 County Data
      addTracts(df = tract_merge[[1]], group  = groups[1], fc = ~pal(average_Income)) %>%     #2012 Tract Data
      
      # addCounties(df = county_merge[[2]], group = groups[3], fc = ~pal(average_Income)) %>%   #2013 County Data
      addTracts(df = tract_merge[[2]], group  = groups[2], fc = ~pal(average_Income)) %>%     #2013 Tract Data

      # addCounties(df = county_merge[[3]], group = groups[5], fc = ~pal(average_Income)) %>%   #2014 County Data
      addTracts(df = tract_merge[[3]], group  = groups[3], fc = ~pal(average_Income)) %>%     #2014 Tract Data

      # addCounties(df = county_merge[[4]], group = groups[7], fc = ~pal(average_Income)) %>%   #2015 County Data
      addTracts(df = tract_merge[[4]], group  = groups[4], fc = ~pal(average_Income)) %>%     #2015 Tract Data

      # addCounties(df = county_merge[[5]], group = groups[9], fc = ~pal(average_Income)) %>%   #2016 County Data
      addTracts(df = tract_merge[[5]], group  = groups[5], fc = ~pal(average_Income)) %>%    #2016 Tract Data

      # addCounties(df = county_merge[[6]], group = groups[11], fc = ~pal(average_Income)) %>%  #2017 County Data
      addTracts(df = tract_merge[[6]], group  = groups[6], fc = ~pal(average_Income)) %>%    #2017 Tract Data

      # addCounties(df = county_merge[[7]], group = groups[13], fc = ~pal(average_Income)) %>%  #2018 County Data
      addTracts(df = tract_merge[[7]], group  = groups[7], fc = ~pal(average_Income)) %>%    #2018 Tract Data
      ...
      addLegend(pal = pal,                                          # Use given color palette
                ...
                title = "Average Individual Median Income",         # Legend Title
...
readFiles <- function(){
   ...
   #files <- list.files(path="./",pattern="*_Med_Income.csv",full.names=T)
...
getMedianTracts <- function(ald, myTracts){
   ...
   for(i in 1:length(ald)){
      ...
      tract_merge[[i]]$average_Income <- as.numeric(tract_merge[[i]]$average_Income)
   }
   
   return(tract_merge)
}

# getMedianCounts <- function(ald, myCounts){
...
#       
#       # Make usable data points numeric rather than chars
#       county_merge[[i]]$average_Income <- as.numeric(county_merge[[i]]$average_Income)
#    }
#    return(county_merge)
# }
...
main <- function(){
  ...
  ### GLOBAL VARS COMMENTED OUT ###
   ...
   myDomain <- minRelevantIncome:max(as.numeric(ald[[7]]$average_Income),na.rm = T)
   ...
   # county_merge <- getMedianCounts(ald, myCounts)
   tract_merge <- getMedianTracts(ald, myTracts)
   
   
   plotAvInc( tract_merge = tract_merge, groups = groups, pal = pal, myDomain = myDomain, cityLng = cityLng,
