...
SDdata <- read.csv("./Education Quality Index.csv", header = T)
...
SD_merge$INDEX <- as.numeric(SD_merge$INDEX)
...
myDomain <- c(minRelevantINDEX:max(as.numeric(SD_merge$INDEX),na.rm = T))
...
map3<-leaflet() %>%
  ...
  addLegend(pal = pal,                               # Use given color palette
            ...
            title = "Quality Index"        # Legend Title
