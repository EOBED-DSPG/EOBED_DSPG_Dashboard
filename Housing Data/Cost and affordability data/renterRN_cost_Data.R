...
##### 
##### File: renterRN_cost_Data.R
##### Author: Collin Robinson
##### Date Created: August 2, 2020
##### Last Update: August 6, 2020

# Map showing percentage of income spent by Registered Nurses on rent by tract/county.
...
# Add counties to map
# addRnRentCostCounts <- function(map, df, group = NULL, fc){
#    # Set up highlight label text (HTML formatting)
#    percentLab <- paste("Percent of RN Income Spent on Rent: ",round(df$RN_income_rent,2),"%",sep = "")
#    amountLab <- paste("Average Rent: $", df$Rent, sep = "")
#    incAm <- paste("RN Income: $", df$RN_income)
#    #moeLab <- paste("Margin of Error on Amount Spent: ",round(df$MOE_Rent,2),"%",sep = "")
#    labels <- sprintf(
...
# Add tracts to map
addRnRentCostTracts <- function(map, df, group = NULL, fc){
   # Set up highlight label text (HTML formatting)
   percentLab <- paste("Percent of RN Income Spent on Rent: ",round(df$RN_income_rent,2),"%",sep = "")
   amountLab <- paste("Average Rent: $", df$Rent, sep = "")
   incAm <- paste("RN Income: $", df$RN_income)
...
plotRnRentCost <- function( tract_merge, groups = NULL, pal, myDomain, cityLng,
   ...
   ownerMap<-leaflet() %>%
      ...
      # addRnRentCostCounts(df = county_merge[[1]], group = groups[1], fc = ~pal(RN_income_rent)) %>%   #2010 County Data
      addRnRentCostTracts(df = tract_merge[[1]], group  = groups[1], fc = ~pal(RN_income_rent)) %>%     #2010 Tract Data
      
      # addRnRentCostCounts(df = county_merge[[2]], group = groups[3], fc = ~pal(RN_income_rent)) %>%   #2011 County Data
      addRnRentCostTracts(df = tract_merge[[2]], group  = groups[2], fc = ~pal(RN_income_rent)) %>%     #2011 Tract Data
      
      # addRnRentCostCounts(df = county_merge[[3]], group = groups[5], fc = ~pal(RN_income_rent)) %>%   #2012 County Data
      addRnRentCostTracts(df = tract_merge[[3]], group  = groups[3], fc = ~pal(RN_income_rent)) %>%     #2012 Tract Data
      
      # addRnRentCostCounts(df = county_merge[[4]], group = groups[7], fc = ~pal(RN_income_rent)) %>%   #2013 County Data
      addRnRentCostTracts(df = tract_merge[[4]], group  = groups[4], fc = ~pal(RN_income_rent)) %>%     #2013 Tract Data
      
      # addRnRentCostCounts(df = county_merge[[5]], group = groups[9], fc = ~pal(RN_income_rent)) %>%   #2014 County Data
      addRnRentCostTracts(df = tract_merge[[5]], group  = groups[5], fc = ~pal(RN_income_rent)) %>%    #2014 Tract Data
      
      # addRnRentCostCounts(df = county_merge[[6]], group = groups[11], fc = ~pal(RN_income_rent)) %>%  #2015 County Data
      addRnRentCostTracts(df = tract_merge[[6]], group  = groups[6], fc = ~pal(RN_income_rent)) %>%    #2015 Tract Data
      
      # addRnRentCostCounts(df = county_merge[[7]], group = groups[13], fc = ~pal(RN_income_rent)) %>%  #2016 County Data
      addRnRentCostTracts(df = tract_merge[[7]], group  = groups[7], fc = ~pal(RN_income_rent)) %>%    #2016 Tract Data
      
      # addRnRentCostCounts(df = county_merge[[8]], group = groups[15], fc = ~pal(RN_income_rent)) %>%  #2017 County Data
      addRnRentCostTracts(df = tract_merge[[8]], group  = groups[8], fc = ~pal(RN_income_rent)) %>%    #2017 Tract Data
      
      # addRnRentCostCounts(df = county_merge[[9]], group = groups[17], fc = ~pal(RN_income_rent)) %>%  #2018 County Data
      addRnRentCostTracts(df = tract_merge[[9]], group  = groups[9], fc = ~pal(RN_income_rent)) %>%    #2018 Tract Data
      ...
      addLegend(pal = pal,                                          # Use given color palette
                ...
                title = "Percent of RN Income Spent on Rent",         # Legend Title
...
getRNCostTracts <- function(myTracts,ald=NULL){
   ...
   for(i in 1:length(ald)){
      ...
      tract_merge[[i]]$RN_income_rent <- as.numeric(tract_merge[[i]]$RN_income_rent)
...
# getRNCostCounts <- function(myCounts, ald=NULL){
#    
...
#       
#       # Make usable data points numeric rather than chars
#       county_merge[[i]]$RN_income_rent <- as.numeric(county_merge[[i]]$RN_income_rent)
...
drawRnRentCostMap <- function(tract, county){
   ###### Set Variables ######
   ...
   # )
   # 
   # coRentRn_merge <- getRNCostCounts(myCounts, county)
   # trRentRn_merge <- getRNCostTracts(myTracts, tract)
   # 
   # 
   # plotRnRentCost(county_merge = coRentRn_merge, tract_merge = trRentRn_merge, groups = groups, pal = pal, myDomain = myDomain, cityLng = cityLng,
