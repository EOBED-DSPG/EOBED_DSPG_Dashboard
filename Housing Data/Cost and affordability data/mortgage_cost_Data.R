...
##### 
##### File: mortgage_cost_Data.R
##### Author: Collin Robinson
##### Date Created: August 2, 2020
##### Last Update: August 6, 2020

# Map showing percentage of income spent on Mortgage by tract/county.
...
# # Add counties to map
# addHOCCostCounts <- function(map, df, group = NULL, fc){
#    # Set up highlight label text (HTML formatting)
#    percentLab <- paste("Percent of Income Spent on Mortgage: ",round(df$median_inc_mort,2),"%",sep = "")
#    amountLab <- paste("Average Mortgage: $", df$HOC, sep = "")
...
# Add tracts to map
addHOCCostTracts <- function(map, df, group = NULL, fc){
   # Set up highlight label text (HTML formatting)
   percentLab <- paste("Percent of Income Spent on Mortgage: ",round(df$median_inc_mort,2),"%",sep = "")
   amountLab <- paste("Average Mortgage: $", df$HOC, sep = "")
   incAm <- paste("Median Household Income: $", df$MedianincHH)
   ...
   addPolygons(map = map,
               ...
               fillOpacity = 1,                        # Make tracts opaque to diffeHOCiate
...
plotHOCCost <- function( tract_merge, groups = NULL, pal, myDomain, cityLng,
   ...
   ownerMap<-leaflet() %>%
      ...
      # addHOCCostCounts(df = county_merge[[1]], group = groups[1], fc = ~pal(median_inc_mort)) %>%   #2010 County Data
      addHOCCostTracts(df = tract_merge[[1]], group  = groups[1], fc = ~pal(median_inc_mort)) %>%     #2010 Tract Data
      
      # addHOCCostCounts(df = county_merge[[2]], group = groups[3], fc = ~pal(median_inc_mort)) %>%   #2011 County Data
      addHOCCostTracts(df = tract_merge[[2]], group  = groups[2], fc = ~pal(median_inc_mort)) %>%     #2011 Tract Data
      
      # addHOCCostCounts(df = county_merge[[3]], group = groups[5], fc = ~pal(median_inc_mort)) %>%   #2012 County Data
      addHOCCostTracts(df = tract_merge[[3]], group  = groups[3], fc = ~pal(median_inc_mort)) %>%     #2012 Tract Data
      
      # addHOCCostCounts(df = county_merge[[4]], group = groups[7], fc = ~pal(median_inc_mort)) %>%   #2013 County Data
      addHOCCostTracts(df = tract_merge[[4]], group  = groups[4], fc = ~pal(median_inc_mort)) %>%     #2013 Tract Data
      
      # addHOCCostCounts(df = county_merge[[5]], group = groups[9], fc = ~pal(median_inc_mort)) %>%   #2014 County Data
      addHOCCostTracts(df = tract_merge[[5]], group  = groups[5], fc = ~pal(median_inc_mort)) %>%    #2014 Tract Data
      
      # addHOCCostCounts(df = county_merge[[6]], group = groups[11], fc = ~pal(median_inc_mort)) %>%  #2015 County Data
      addHOCCostTracts(df = tract_merge[[6]], group  = groups[6], fc = ~pal(median_inc_mort)) %>%    #2015 Tract Data
      
      # addHOCCostCounts(df = county_merge[[7]], group = groups[13], fc = ~pal(median_inc_mort)) %>%  #2016 County Data
      addHOCCostTracts(df = tract_merge[[7]], group  = groups[7], fc = ~pal(median_inc_mort)) %>%    #2016 Tract Data
      
      # addHOCCostCounts(df = county_merge[[8]], group = groups[15], fc = ~pal(median_inc_mort)) %>%  #2017 County Data
      addHOCCostTracts(df = tract_merge[[8]], group  = groups[8], fc = ~pal(median_inc_mort)) %>%    #2017 Tract Data
      
      # addHOCCostCounts(df = county_merge[[9]], group = groups[17], fc = ~pal(median_inc_mort)) %>%  #2018 County Data
      addHOCCostTracts(df = tract_merge[[9]], group  = groups[9], fc = ~pal(median_inc_mort)) %>%    #2018 Tract Data
      ...
      # Ontario city marker (Needs own Marker to point textbox in diffeHOC direction)
      ...
      addPolylines(data = stateM,
                   ...
                   fillOpacity = 0,                       # Make tracts opaque to diffeHOCiate
                   opacity = 1,                           # Make tract borders opaque
      ...
      addLegend(pal = pal,                                          # Use given color palette
                ...
                title = "Percent of Income Spent on HOC",         # Legend Title
...
getMortCostTracts <- function(myTracts, ald=NULL){
   ...
   for(i in 1:length(ald)){
      ...
      tract_merge[[i]]$median_inc_mort <- as.numeric(tract_merge[[i]]$median_inc_mort)
...
# getMortCostCounts <- function(myCounts,ald=NULL){
#    
...
#       
#       # Make usable data points numeric rather than chars
#       county_merge[[i]]$median_inc_mort <- as.numeric(county_merge[[i]]$median_inc_mort)
...
drawHOCCostMap <- function(tract, county){
   ###### Set Variables ######
   ...
   # )
   # 
   # countyMort_merge <- getMortCostCounts(myCounts,county)
   # tractMort_merge <- getMortCostTracts(myTracts,tract)
   # 
   # 
   # plotHOCCost(county_merge = countyMort_merge, tract_merge = tractMort_merge, groups = groups, pal = pal, myDomain = myDomain, cityLng = cityLng,
