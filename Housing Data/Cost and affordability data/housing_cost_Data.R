...
addRentCostCounts <- function(map, df, group = NULL, fc){
   ...
   incAm <- paste("RN Income: $", df$MedianincHH)
   #moeLab <- paste("Margin of Error on Amount Spent: ",round(df$MOE_HOC,2),"%",sep = "")
   labels <- sprintf(
      "<strong>%s</strong><br/>%s<br/>%s",df$Geographic.Area.Name,percentLab,amountLab#moeLab
   )
   # Add Shapes to map
   addPolygons(map = map,
               data = df,
...
plotRentCost <- function(county_merge, tract_merge, groups = NULL, pal, myDomain, cityLng,
   ...
   ownerMap<-leaflet() %>%
      ...
      addRentCostTracts(df = tract_merge[[1]], group  = groups[2], fc = ~pal(median_inc_rent)) %>%     #2010 Tract Data
      
      addRentCostCounts(df = county_merge[[2]], group = groups[3], fc = ~pal(median_inc_rent)) %>%   #2011 County Data
      addRentCostTracts(df = tract_merge[[2]], group  = groups[4], fc = ~pal(median_inc_rent)) %>%     #2011 Tract Data
      
      addRentCostCounts(df = county_merge[[3]], group = groups[5], fc = ~pal(median_inc_rent)) %>%   #2012 County Data
      addRentCostTracts(df = tract_merge[[3]], group  = groups[6], fc = ~pal(median_inc_rent)) %>%     #2012 Tract Data
      
      addRentCostCounts(df = county_merge[[4]], group = groups[7], fc = ~pal(median_inc_rent)) %>%   #2013 County Data
      addRentCostTracts(df = tract_merge[[4]], group  = groups[8], fc = ~pal(median_inc_rent)) %>%     #2013 Tract Data
      
      addRentCostCounts(df = county_merge[[5]], group = groups[9], fc = ~pal(median_inc_rent)) %>%   #2014 County Data
      addRentCostTracts(df = tract_merge[[5]], group  = groups[10], fc = ~pal(median_inc_rent)) %>%    #2014 Tract Data
      
      addRentCostCounts(df = county_merge[[6]], group = groups[11], fc = ~pal(median_inc_rent)) %>%  #2015 County Data
      addRentCostTracts(df = tract_merge[[6]], group  = groups[12], fc = ~pal(median_inc_rent)) %>%    #2015 Tract Data
      
      addRentCostCounts(df = county_merge[[7]], group = groups[13], fc = ~pal(median_inc_rent)) %>%  #2016 County Data
      addRentCostTracts(df = tract_merge[[7]], group  = groups[14], fc = ~pal(median_inc_rent)) %>%    #2016 Tract Data
      
      addRentCostCounts(df = county_merge[[8]], group = groups[15], fc = ~pal(median_inc_rent)) %>%  #2017 County Data
      addRentCostTracts(df = tract_merge[[8]], group  = groups[16], fc = ~pal(median_inc_rent)) %>%    #2017 Tract Data
      
      addRentCostCounts(df = county_merge[[9]], group = groups[17], fc = ~pal(median_inc_rent)) %>%  #2018 County Data
      addRentCostTracts(df = tract_merge[[9]], group  = groups[18], fc = ~pal(median_inc_rent)) %>%    #2018 Tract Data
...
getCostTracts <- function(myTracts){
   
   files <- list.files(path="./",pattern="*Tract_Data_Housing.csv",full.names=T)
...
getCostCounts <- function(myCounts){
   
   files <- list.files(path="./",pattern="*County_Data_Housing.csv",full.names=T)
   ald <- sapply(files, read.csv, skip = 1, header = T, simplify = F)
...
drawRentCostMap <- function(){
   ...
   county_merge <- getCostCounts(myCounts)
   tract_merge <- getCostTracts(myTracts)
   
   
   plotRentCost(county_merge = county_merge, tract_merge = tract_merge, groups = groups, pal = pal, myDomain = myDomain, cityLng = cityLng,
