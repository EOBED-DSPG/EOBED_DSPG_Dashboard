File(s): 

	2018_Tract_Data_DeterminantsHealth.csv



Used Column(s): 

	Geographic Area Name, id_Fix, INDEX_Overall, INDEX_Insurance, INDEX_AccesstoFood

Maps to make:
	Heathcare Outcomes and Quality (index map using INDEX_Overall data)
	Health insurance coverage (INDEX_Insurance)
`	Access to food (INDEX_AccesstoFood)
	
Table columns:	
	County
	Census Tract
	Ranking Overall
	Quintile
	Food Desert?
	% Insured


Notes:
	Generally speaking, 1 is good, 5 is bad. 




Column Description:

	id_Fix: 
		Usable part of GEO_ID for leaflet maps

	Geographic Area Name: 
		County, Tract, or State name (ie Malheur County, `Census Tract 201, Canyon County, Idaho`, etc.)

	INDEX_Overall:
		The overall index, values range from 1-5 (where 5=the bottom quintile/worst, and 1= top quintile/best)